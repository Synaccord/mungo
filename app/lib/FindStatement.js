...
class FindStatement {
    ...
    for (let field in document) {
      if (document[field] instanceof Promise) {
        ...
        case '$nor' :
          ...
          parsed[field] = document[field].map(
            value => this.parseAll(value, structure)
          );
          break;
  ...
  parseField(fieldName, fieldValue, fieldStructure, schema) {
    let parsedFieldStructure = fieldStructure;
    try {
      if (/\./.test(fieldName)) {
      ...
      if (fieldValue && typeof fieldValue === 'object') {
        ...
        if (FindStatement.operators.indexOf(key) > -1) {
          ...
          case '$size':
          case '$slice':
            return {[key]: parsedFieldStructure.type.convert(value)};
          ...
          case '$nin':
            return {[key]: value.map(
              valueItem => parsedFieldStructure.type.convert(valueItem))
            };

          case '$not':
          case '$elemMatch':
            return {[key]: this.parseField(
              fieldName, value, parsedFieldStructure
            )};
          ...
          case '$mod':
            ...
            return {[key]: value.map(item => Type.Number.convert(item))};
          ...
          case '$text':
            ...
            if (typeof value === 'string') {
              ...
              throw new MungoError(
                'FindStatement:$text >' +
                  ' value must be either a string or an object',
                {value}
              );
            }
      ...
      if (!parsedFieldStructure || !('type' in parsedFieldStructure)) {
        console.log('parse field error',
          {fieldName, fieldValue, parsedFieldStructure, schema}
        );
      }

      return parsedFieldStructure.type.convert(fieldValue);
    } catch (error) {
      throw MungoFindStatementError.rethrow(
        error,
        'Can not parse field of find statement',
        {fieldName, fieldValue, parsedFieldStructure}
      );
    }
  }
